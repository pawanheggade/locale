...
import { DisplayablePost, PostActions, NotificationSettings, Notification, Account, AdminView, Post, PostType, ForumPost, ForumComment, Feedback } from './types';
...
export const App: React.FC = () => {
  const { 
    ...
    bag, 
    addPostToViewHistory, viewedPostIds,
  ...
  const { 
    posts: allDisplayablePosts, archivedPosts, categories, createPost: createPostInContext, updatePost: updatePostInContext, archivePost, unarchivePost, deletePostPermanently, addCategory, updateCategory, deleteCategory, findPostById,
    togglePinPost, priceUnits, addPriceUnit, updatePriceUnit, deletePriceUnit, refreshPosts
  } = usePosts();
  
  const { posts: forumPosts, getPostWithComments, findForumPostById, categories: forumCategories, addCategory: addForumCategory, updateCategory: updateForumCategory, deleteCategory: deleteForumCategory } = useForum();
  const { activeModal, openModal, closeModal, addToast } = useUI();
  const { filterState } = useFilters();
  const { view, mainView, viewingPostId, viewingAccount, viewingForumPostId, mainContentRef, navigateTo } = useNavigation();
  ...
  useEffect(() => {
    if (view === 'postDetail' && viewingPostId && currentAccount) {
      addPostToViewHistory(viewingPostId);
    }
    if (view === 'account' && viewingAccount) {
        incrementProfileViews(viewingAccount.id);
    }
  }, [view, viewingPostId, viewingAccount, currentAccount, addPostToViewHistory, incrementProfileViews]);
  ...
  const performSignOut = useCallback(() => {
    authSignOut();
    // Reset view state via navigation
    navigateTo('all');
  }, [authSignOut, navigateTo]);
  ...
  const postActions: PostActions = useMemo(() => ({
    onToggleLikePost: withAuthCheck((postId: string) => toggleLikePost(postId)),
    onArchive: withAuthCheck(archivePost),
    onUnarchive: withAuthCheck(unarchivePost),
    onDeletePermanently: withAuthCheck(deletePostPermanently),
    onEdit: withAuthCheck((postId: string) => navigateTo('editPost', { postId })),
    ...
    onReportItem: withAuthCheck((item: Post | ForumPost | ForumComment) => openModal({ type: 'reportItem', data: { item } })),
    onSetPriceAlert: withAuthCheck((postId: string) => { const post = findPostById(postId); if (post) openModal({ type: 'setPriceAlert', data: post }); }),
    ...
    onAddToBag: withAuthCheck((postId: string) => { const post = findPostById(postId); if (post) openModal({ type: 'addToBag', data: post }); }),
    ...
    onToggleLikeAccount: withAuthCheck((account: Account) => toggleLikeAccount(account.id)),
    onTogglePinPost: withAuthCheck(togglePinPost),
  }), [
    withAuthCheck, openModal, archivePost, unarchivePost, deletePostPermanently, navigateTo, findPostById,
    accountsById, availabilityAlerts, setAvailabilityAlert, deleteAvailabilityAlert,
    priceAlerts, toggleLikePost, toggleLikeAccount, showConfirmation,
    deletePriceAlert, setPriceAlert
  ]);
  ...
  const { isAnyFilterActive } = useFilters();
  ...
  const renderCurrentView = () => {
    switch (view) {
        case 'all':
            if (mainView === 'grid') return <div className="p-4 sm:p-6 lg:p-8">{showRecommendations && <div className="mb-8 animate-fade-in-up"><PostList posts={recommendations} currentAccount={currentAccount} isLoading={false} variant={gridView} enableEntryAnimation={true} /></div>}<PostList posts={displayPosts} currentAccount={currentAccount} onLoadMore={loadMore} hasMore={hasMore} isLoadingMore={isLoadingMore} isLoading={isInitialLoading} isFiltering={filterState.isAiSearching} variant={gridView} enableEntryAnimation={true} /></div>;
            if (mainView === 'map') return <Suspense fallback={<LoadingFallback/>}><MapView posts={displayPosts} /></Suspense>;
            return null;
        ...
        case 'bag': return currentAccount ? <Suspense fallback={<LoadingFallback/>}><BagView allAccounts={accounts} /></Suspense> : null;
        case 'admin':
            return currentAccount?.role === 'admin' ? <Suspense fallback={<LoadingFallback/>}><AdminPanel accounts={accounts} allPosts={allDisplayablePosts} currentAccount={currentAccount} onDeleteAccount={deleteAccount} onUpdateAccountRole={updateAccountRole} onEditAccount={(acc) => openModal({ type: 'editAccount', data: acc })} onToggleAccountStatus={(acc) => toggleAccountStatus(acc.id, true)} onApproveAccount={(id) => { approveAccount(id); addNotification({ recipientId: id, message: 'Your account has been approved.', type: 'account_approved' }); }} onRejectAccount={(acc) => rejectAccount(acc.id)} categories={categories} onAddCategory={addCategory} onUpdateCategory={updateCategory} onDeleteCategory={deleteCategory} onUpdateSubscription={updateSubscription} reports={reports} onReportAction={(report, action) => { if(action==='delete') { /* delete logic handled in context */ } setReports(prev => prev.filter(r => r.id !== report.id)); addToast('Report handled.', 'success'); }} feedbackList={feedbackList} onDeleteFeedback={handleDeleteFeedback} onToggleFeedbackArchive={handleToggleFeedbackArchive} onMarkFeedbackAsRead={handleMarkFeedbackAsRead} onBulkFeedbackAction={handleBulkFeedbackAction} termsContent={termsContent} onUpdateTerms={setTermsContent} privacyContent={privacyContent} onUpdatePrivacy={setPrivacyContent} initialView={adminInitialView} forumPosts={forumPosts} getPostWithComments={getPostWithComments} priceUnits={priceUnits} onAddPriceUnit={addPriceUnit} onUpdatePriceUnit={updatePriceUnit} onDeletePriceUnit={deletePriceUnit} /></Suspense> : null;
        case 'account': return viewingAccount ? <Suspense fallback={<LoadingFallback/>}><AccountView account={viewingAccount} currentAccount={currentAccount} posts={allDisplayablePosts} archivedPosts={archivedPosts} allAccounts={accounts} isLiked={currentAccount?.likedAccountIds?.includes(viewingAccount.id) ?? false} /></Suspense> : null;
        case 'postDetail': return viewingPost ? <PostDetailView post={viewingPost} currentAccount={currentAccount} /> : null;
        case 'forums': return <Suspense fallback={<LoadingFallback/>}><ForumsView /></Suspense>;
        case 'forumPostDetail': return viewingForumPostId ? <ForumsPostDetailView postId={viewingForumPostId} /> : null;
        case 'createPost':
            if (currentAccount?.subscription.tier === 'Personal') { navigateTo('subscription'); addToast('You need a seller account.', 'error'); return null; }
            return currentAccount ? <CreatePostPage onSubmitPost={(d) => createPostInContext(d, currentAccount!.id)} currentAccount={currentAccount} categories={categories} onUpdateCurrentAccountDetails={(data) => updateAccountDetails({ ...currentAccount, ...data })} /> : null;
        case 'editPost': return currentAccount && viewingPost ? <CreatePostPage onUpdatePost={updatePostInContext} editingPost={viewingPost} currentAccount={currentAccount} categories={categories} /> : null;
        ...
        <main ref={mainContentRef} onScroll={handleScroll} className={cn("relative flex-1 overflow-y-auto bg-gray-50 pt-16", isInitialLoading && "overflow-hidden", isPulling && "overflow-hidden")}>
          {view === 'all' && mainView === 'grid' ? (
            <>
             ...
             <ErrorBoundary><Suspense fallback={<LoadingFallback/>}>{renderCurrentView()}</Suspense></ErrorBoundary>
          )}
        </main>
        <OfflineIndicator />
