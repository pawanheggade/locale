...
export const AccountForm: React.FC<AccountFormProps> = ({ account, isEditing, allAccounts, onSubmit, formId, isSubmitting, isSellerSignup = false, onToggleMap, isUpgrading = false }) => {
    ...
    const handleSellerOptionChange = (field: keyof SellerOptionsState, value: any) => {
        // Auto-fill messageNumber from mobile if 'message' option is selected and messageNumber is empty
        if (field === 'contactOptions') {
            const newOptions = value as ContactOption[];
            const oldOptions = state.sellerOptions.contactOptions;
            const isMessageSelected = newOptions.includes('message');
            const wasMessageSelected = oldOptions.includes('message');

            if (isMessageSelected && !wasMessageSelected && !state.messageNumber && state.mobile) {
                dispatch({ type: 'SET_FIELD', field: 'messageNumber', value: state.mobile });
            }
        }
    ...
    const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: 'mobile' | 'messageNumber') => {
        ...
        // Auto-fill messageNumber if typing mobile and messageNumber is empty
        if (fieldName === 'mobile' && !state.messageNumber) {
             handleFieldChange('messageNumber', val);
        }
    };
