  ...
  if (!isOwnPost) {
    menuItems.push(
      <DropdownMenuItem
        ...
        icon={<FlagIcon className="w-5 h-5 text-gray-600" />}
        label="Report"
      />
    );
  }
    ...
    if (!isArchived && !isOwnPost) {
      adminItems.push(
        <DropdownMenuItem
          ...
          icon={<PencilIcon className="w-5 h-5 text-gray-600" />}
          label="Edit"
        />
      );
    }
  ...
  return (
    <div className={`relative ${wrapperClassName}`} ref={menuRef}>
      <Button
        ...
        title="More options"
        className={
            variant === 'modal' ? 'text-gray-600' :
            (variant === 'overlay' ? 'text-white' : 'text-gray-600')
        }
      >
